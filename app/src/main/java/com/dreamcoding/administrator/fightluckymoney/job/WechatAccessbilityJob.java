...
import com.dreamcoding.administrator.fightluckymoney.BuildConfig;
...
import com.dreamcoding.administrator.fightluckymoney.interf.IStatusBarNotification;
import com.dreamcoding.administrator.fightluckymoney.service.QiangHongBaoService;
import com.dreamcoding.administrator.fightluckymoney.util.AccessibilityHelper;
import com.dreamcoding.administrator.fightluckymoney.util.NotifyHelper;
...
public class WechatAccessbilityJob extends BaseAccessbilityJob {
    ...
    private void newHongBaoNotification(Notification notification) {
        ...
        if(lock || getConfig().getWechatMode() != QianghongbaoWechatConfig.WX_MODE_0) {
    ...
    private void openHongBao(AccessibilityEvent event) {
        if("com.tencent.mm.plugin.luckymoney.ui.LuckyMoneyReceiveUI".equals(event.getClassName())) {
            ...
            //拆完红包后看详细的纪录界面
            if(getConfig().getWechatAfterGetHongBaoEvent() == QianghongbaoWechatConfig.WX_AFTER_GET_GOHOME) { //返回主界面，以便收到下一次的红包通知
    ...
    private void handleLuckyMoneyReceive() {
        ...
        if(event == QianghongbaoWechatConfig.WX_AFTER_OPEN_HONGBAO) { //拆红包
        ...
        if(targetNode != null) {
            ...
            if(event == QianghongbaoWechatConfig.WX_AFTER_OPEN_HONGBAO) {
    ...
    private void handleChatListHongBao() {
        ...
        if(mode == QianghongbaoWechatConfig.WX_MODE_3) { //只通知模式
            return;
        }
        ...
        if(mode != QianghongbaoWechatConfig.WX_MODE_0) {
            boolean isMember = isMemberChatUi(nodeInfo);
            if(mode == QianghongbaoWechatConfig.WX_MODE_1 && isMember) {//过滤群聊
                return;
            } else if(mode == QianghongbaoWechatConfig.WX_MODE_2 && !isMember) { //过滤单聊
                return;
            }
        }
